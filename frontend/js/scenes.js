...
/** Game-world constants — tuned for the 480×270 viewport. */
const WORLD = {
    width: 2800,
    groundY: 222,
    gravity: 980,
...
function buildPlayingContext(opts) {
    ...
    const enemyActiveDistance = opts.enemyActiveDistance ?? 300;

    // ----- Player entity ----------------------------------------------------
    const p = k.add([
        k.sprite('playerIdle'),
        k.pos(opts.spawnX, WORLD.groundY - 24),
    ...
    p.onUpdate(() => {
        ...
        const moving = Math.abs(p.vel.x) > 10;
        const grounded = p.isGrounded();
        let spriteName;
        ...
        if (p.pos.y > k.height() + 60) playerDeath();
    });
    ...
    function useWeapon() {
        ...
        if (wp === 'fists' || wp === 'bat') {
            ...
            const isBat = wp === 'bat';
            const reach = isBat ? 10 : 6;
            const base = isBat ? 45 : 25;
            const damage = k.time() < player.rageUntil ? base * 2 : base;
            // Hitbox starts at the player's body edge (±8 from center, since the
            // 16-wide sprite uses anchor 'top') and extends `reach` px outward.
            const rx = p.pos.x + (player.facing > 0 ? 8 : -(8 + reach));
            const ry = p.pos.y + 4;
            k.add([
                k.rect(reach, 18),
                k.pos(rx, ry),
            ...
            if (isBat) {
                ...
                // the 12×8 sprite; angles below are chosen so the barrel arcs
                ...
                const batVis = k.add([
                    k.sprite('bat'),
                    k.pos(p.pos.x + facing * 2, p.pos.y + 10),
                    k.anchor('left'),
                    k.rotate(swingStart),
                    k.opacity(1),
                    k.z(10),
                ...
                batVis.onUpdate(() => {
                    ...
                    batVis.pos.x = p.pos.x + batVis.swingFacing * 2;
                    batVis.pos.y = p.pos.y + 10;
                });
            }
        } else if (wp === 'handgun') {
            if (player.ammo.handgun <= 0) return;
            const dir = player.facing;
            const bx = p.pos.x + (dir > 0 ? 14 : -10);
            const by = p.pos.y + 10;
            // Intercept: player is point-blank at door — bullets are fast enough
            // to tunnel through the 10px hitbox, so we hit the door directly.
            const doorTarget = k.get('door')[0];
            if (doorTarget && dir === 1 &&
                p.pos.x < doorTarget.pos.x + 10 && p.pos.x > doorTarget.pos.x - 35) {
                player.ammo.handgun--;
                player.lastAttack = k.time();
                doorTarget.hurt(k.time() < player.rageUntil ? 80 : 40);
                k.add([k.sprite('flash'), k.pos(bx, by - 2), k.anchor('center'), k.opacity(1), k.lifespan(0.06)]);
                return;
            }
            player.ammo.handgun--;
            player.lastAttack = k.time();
            ...
            if (doorTarget && dir === 1 &&
                ...
                player.lastAttack = k.time();
                doorTarget.hurt(40);
                if (opts.spawnFireAt) opts.spawnFireAt(doorTarget.pos.x + 5);
                return;
            }
            player.ammo.molotov--;
            stats.arsonCount++;
            ...
            bottle.onUpdate(() => {
                ...
                if (bottle.pos.y >= WORLD.groundY - 4) {
            ...
            k.add([k.sprite('flashBig'), k.pos(bx + dir * 4, by - 2), k.anchor('center'), k.opacity(1), k.lifespan(0.1), k.z(40)]);
            if (k.shake) k.shake(2);
        } else if (wp === 'smg') {
            ...
            const dir = player.facing;
            const bx = p.pos.x + (dir > 0 ? 14 : -10);
            const by = p.pos.y + 10 + (Math.random() - 0.5) * 2;
            const b = k.add([
                k.sprite('bullet', { flipX: dir < 0 }),
                k.pos(bx, by),
                k.area(),
            ...
            k.add([k.sprite('flash'), k.pos(bx, by - 2), k.anchor('center'), k.opacity(1), k.lifespan(0.04), k.z(40)]);
        } else if (wp === 'taser') {
            ...
            for (let i = 0; i < 4; i++) {
                ...
                const flame = k.add([
                    ...
                    k.pos(bx + dir * Math.random() * 6, by - 2 + Math.random() * 6),
                    k.area(),
                    k.anchor('center'),
                    k.opacity(1),
                    k.lifespan(0.35),
                    'flameBolt',
                    { damage: 9, vx: dir * speed, vy: spread * speed + 60 },
                ]);
                flame.onUpdate(() => {
                    flame.pos.x += flame.vx * k.dt();
                    flame.pos.y += flame.vy * k.dt();
                    flame.vy += 180 * k.dt();
                    if (flame.pos.y >= WORLD.groundY - 2) {
            ...
            nade.onUpdate(() => {
                ...
                if (nade.pos.y >= WORLD.groundY - 3) {
                    nade.pos.y = WORLD.groundY - 3;
                    nade.vy = -Math.abs(nade.vy) * 0.3;
                    nade.vx *= 0.5;
                }
                if (nade.fuse <= 0) {
                    explode(nade.pos.x, nade.pos.y - 4);
                    k.destroy(nade);
                }
            });
        }
    ...
    function spawnBloodSplatter(x, y, dirX, intensity) {
        ...
        for (let i = 0; i < count; i++) {
            const speedX = dirX * (40 + Math.random() * 160) + (Math.random() - 0.5) * 80;
            const speedY = -(60 + Math.random() * 160);
            const droplet = k.add([
                k.sprite('blood'),
                k.pos(x + (Math.random() - 0.5) * 6, y + (Math.random() - 0.5) * 4),
                k.anchor('center'),
                k.z(30),
                k.opacity(1),
                k.lifespan(1.2),
            ...
            droplet.onUpdate(() => {
                ...
                if (droplet.pos.y >= WORLD.groundY - 1) {
                    droplet.pos.y = WORLD.groundY - 1;
                    droplet.landed = true;
                    droplet.vx = 0;
                    droplet.vy = 0;
    ...
    function explode(x, y) {
        ...
        for (const e of k.get('enemy')) {
            const dx = e.pos.x - x;
            const dy = e.pos.y - y;
            if (Math.abs(dx) < radius && Math.abs(dy) < 48) {
                e.hurt(damage);
                if (e.kind !== 'child') e.state = 'hostile';
                spawnBloodSplatter(e.pos.x + 8, e.pos.y + 12, Math.sign(dx) || 1, 1.2);
            }
        }
        const pDx = p.pos.x - x;
    ...
    function spawnFire(x, y) {
        ...
        fire.onUpdate(() => {
            ...
            if (fire.dmgTick > 0.4) {
                fire.dmgTick = 0;
                for (const e of k.get('enemy')) {
                    if (Math.abs(e.pos.x - fire.pos.x) < 14) {
                        e.hurt(12);
                        if (e.state !== 'hostile' && e.kind !== 'child') e.state = 'hostile';
                        spreadAggro(e.pos.x);
                    }
                }
                if (Math.abs(p.pos.x - fire.pos.x) < 14) {
                    hurtPlayer(8, fire);
                }
            }
        });
    ...
    function spreadAggro(x) {
        k.get('enemy').forEach((e) => {
            if (Math.abs(e.pos.x - x) < 180 && e.kind !== 'child') {
                e.state = 'hostile';
            }
        });
    }
    ...
    const enemyDefs = {
        dog:     { hp: 20, spd: 80, dmg: 6,  range: 10, swingCd: 0.8,  spriteBase: 'dog',     h: 12, w: 16, killsKey: 'dog' },
        child:   { hp: 10, spd: 70, dmg: 0,  range: 0,  swingCd: null, spriteBase: 'child',   h: 17, w: 12, killsKey: 'child' },
        father:  { hp: 45, spd: 50, dmg: 15, range: 14, swingCd: 1.2,  spriteBase: 'father',  h: 24, w: 16, killsKey: 'civilian' },
        mother:  { hp: 35, spd: 50, dmg: 12, range: 14, swingCd: 1.4,  spriteBase: 'mother',  h: 24, w: 16, killsKey: 'civilian' },
        scout:   { hp: 35, spd: 55, dmg: 10, range: 14, swingCd: 1.1,  spriteBase: 'scout',   h: 24, w: 16, killsKey: 'civilian' },
        cop:     { hp: 70, spd: 60, dmg: 20, range: 70, swingCd: null, spriteBase: 'cop',     h: 24, w: 16, killsKey: 'cop' },
        mallCop: { hp: 55, spd: 50, dmg: 14, range: 14, swingCd: 1.3,  spriteBase: 'mallCop', h: 24, w: 16, killsKey: 'cop' },
        boss:    { hp: 140,spd: 55, dmg: 25, range: 16, swingCd: 1.0,  spriteBase: 'boss',    h: 24, w: 16, killsKey: 'boss' },
        choir:   { hp: 30, spd: 45, dmg: 10, range: 14, swingCd: 1.5,  spriteBase: 'choir',   h: 24, w: 16, killsKey: 'choir' },
        // SWAT response unit — spawns in reply to a slain cop. Tougher, burst-
        // fires an SMG, always drops the SMG on death.
        swat:    { hp: 110,spd: 60, dmg: 16, range: 70, swingCd: null, spriteBase: 'swat',    h: 24, w: 16, killsKey: 'cop' },
    };
    ...
    function spawnEnemy(kind, x, mode, id) {
        ...
        const e = k.add([
            ...
            'enemy',
            {
                ...
                swingCd: def.swingCd,
                wanderTarget: x + (Math.random() * 80 - 40),
        ...
        e.onUpdate(() => {
            ...
            if (e.state === 'hostile') {
                ...
                e.facing = dir;
                vx = dir * e.speed * 0.5;
                if (Math.abs(e.pos.x - e.wanderTarget) < 4) {
                    e.wanderTarget = e.pos.x + (Math.random() * 80 - 40);
                }
            }
            e.move(vx, 0);
            ...
            if (e.state === 'hostile' && e.swingCd && k.time() - e.lastSwing >= e.swingCd) {
                ...
                if (dist <= e.range && atkDy <= 24) {
                    ...
                    const punch = k.add([
                        ...
                        k.pos(e.pos.x + atkDir * 8, e.pos.y + 12),
                        k.anchor('center'),
                        k.z(50),
                        { punchAge: 0, punchVx: ((p.pos.x - atkDir * 4) - (e.pos.x + atkDir * 8)) / SWING },
                    ]);
                    punch.onUpdate(() => {
                        punch.punchAge += k.dt();
                    ...
                    k.wait(SWING, () => {
                        if (!e.exists()) return;
                        e.color = k.rgb(255, 255, 255);
                        if (Math.abs(e.pos.x - p.pos.x) <= e.range + 4 && Math.abs(e.pos.y - p.pos.y) <= 24) {
            ...
            if ((e.kind === 'cop' || e.kind === 'swat') && e.state === 'hostile' && dist < 220) {
                    ...
                    for (let i = 0; i < burst; i++) {
                        k.wait(i * 0.08, () => {
                            ...
                            const b = k.add([
                                ...
                                k.pos(e.pos.x + bDir * 10, e.pos.y + 8 + (Math.random() - 0.5) * 2),
                                k.anchor('center'),
                                k.offscreen({ destroy: true, distance: 200 }),
                                k.area({ collisionIgnore: ['enemy'] }),
                                'enemyBullet',
        ...
        e.onDeath(() => {
            ...
            spawnPoof(e.pos.x + 8, e.pos.y + def.h / 2);
            ...
            spawnBloodSplatter(e.pos.x + 8, e.pos.y + def.h / 2, deathDir, 2.2);
            let dropOff = 0;
            for (const [item, chance] of (/** @type {any} */ (LOOT_TABLE)[kind] || [])) {
                if (Math.random() < chance) {
                    spawnPickup(item, e.pos.x + dropOff, WORLD.groundY - 6);
                    dropOff += 14;
                }
            }
            ...
            if (kind === 'cop' && !run.killedEnemies.has('swat-response-' + (e.id || k.time()))) {
                ...
                const swatX = p.pos.x + spawnDir * 200;
                k.wait(1.4, () => {
                    if (!p.exists()) return;
        ...
        e.onHurt(() => {
            ...
            spawnBloodSplatter(e.pos.x + 8, e.pos.y + def.h / 2, hitDir, 0.55);
        });
    ...
    function spawnPickup(kind, x, y, id) {
        ...
        item.onUpdate(() => {
            item.pos.y = item.baseY + Math.sin(k.time() * 3 + x) * 1.5;
        ...
        const hint = k.add([
            k.text(pickupHintText(kind), { size: 9 }),
            k.pos(x, y - 20),
            k.anchor('center'),
            k.color(255, 240, 80),
            k.opacity(0),
        ...
        hint.onUpdate(() => {
            ...
            const dx = Math.abs(item.pos.x - (p.pos.x + 8));
            hint.pos.y = item.pos.y - 20 + Math.sin(k.time() * 5) * 1.5;
            hint.opacity = dx < 22 ? 1 : 0;
        });
        return item;
    }
    ...
    function nearestInteractable(maxDx, maxDy) {
        ...
        for (const item of k.get('pickup')) {
            const dx = Math.abs(item.pos.x - (p.pos.x + 8));
            const dy = Math.abs(item.pos.y - (p.pos.y + 16));
    ...
    // ----- HUD (fixed overlay) ---------------------------------------------
    k.add([k.rect(82, 10), k.pos(6, 6), k.color(20, 20, 20), k.fixed(), k.z(100)]);
    const hpFill = k.add([k.rect(80, 8), k.pos(7, 7), k.color(211, 47, 47), k.fixed(), k.z(101)]);
    hpFill.onUpdate(() => {
        hpFill.width = 80 * Math.max(0, player.health / player.maxHealth);
    ...
    k.add([k.text('HP', { size: 12 }), k.pos(92, 7), k.fixed(), k.color(255, 255, 255), k.z(101)]);
    ...
    k.add([k.rect(82, 8), k.pos(6, 18), k.color(20, 20, 20), k.fixed(), k.z(100)]);
    const intoxFill = k.add([k.rect(80, 6), k.pos(7, 19), k.color(230, 120, 20), k.fixed(), k.z(101)]);
    intoxFill.onUpdate(() => {
        ...
        intoxFill.width = 80 * level;
        intoxFill.color = highLeft > 0 ? k.rgb(160, 60, 220) : k.rgb(230, 120, 20);
    });
    const intoxLabel = k.add([k.text('INTOX', { size: 10 }), k.pos(92, 18), k.fixed(), k.color(255, 255, 255), k.z(101)]);
    intoxLabel.onUpdate(() => {
    ...
    function rebuildWeaponHud() {
        ...
        player.weapons.forEach((wpn, i) => {
            ...
            const inner = k.add([
                k.rect(SLOT_SIZE - 2, SLOT_SIZE - 2),
                k.pos(x + 1, y + 1),
                k.color(24, 24, 24),
                k.opacity(1),
                k.fixed(),
            ...
            const ammoLabel = k.add([
                k.text('', { size: 8 }),
                k.pos(x + SLOT_SIZE - 2, y + SLOT_SIZE - 1),
                k.anchor('botright'),
                k.color(255, 255, 255),
                k.fixed(),
                k.z(103),
    ...
    const weaponNameLabel = k.add([
        k.text('', { size: 9 }),
        k.pos(SLOT_BASE_X, SLOT_BASE_Y + SLOT_SIZE + 2),
        k.anchor('topleft'),
        k.color(240, 200, 80),
        k.fixed(),
        k.z(101),
    ...
    const timer = k.add([k.text('0:00', { size: 12 }), k.pos(k.width() - 6, 7), k.anchor('topright'), k.fixed(), k.color(255, 255, 255), k.z(101)]);
    timer.onUpdate(() => {
    ...
    const usaFlash = k.add([k.text('', { size: 14 }), k.pos(k.width() / 2, 10), k.anchor('top'), k.fixed(), k.color(255, 255, 255), k.z(101)]);
    usaFlash.onUpdate(() => {
...
function splashScene() {
    ...
    // Flag bars at top
    k.add([k.rect(k.width(), 6), k.pos(0, 6), k.color(211, 47, 47), k.fixed()]);
    k.add([k.rect(k.width(), 6), k.pos(0, 18), k.color(255, 255, 255), k.fixed()]);
    k.add([k.rect(k.width(), 6), k.pos(0, 30), k.color(42, 79, 184), k.fixed()]);

    // Title
    k.add([
        k.text('B.A.M.', { size: 60 }),
        k.pos(k.width() / 2, 70),
        k.anchor('center'),
        k.color(255, 255, 255),
    ]);
    k.add([
        k.text('BRAVE AMERICA MAN', { size: 13 }),
        k.pos(k.width() / 2, 104),
        k.anchor('center'),
        k.color(220, 220, 220),
    ]);

    // Player sprite standing proud on the left
    k.add([k.sprite('playerIdle'), k.pos(110, 130), k.scale(2), k.anchor('top')]);

    // Opening quote — to the right of the player sprite. Hand-wrapped so the
    // layout stays predictable across kaplay font widths and we can clear the
    // CONTROLS header below.
    const lines = [
    ...
    controls.forEach(([key, action], i) => {
        const y = 210 + i * 11;
        k.add([k.text(key,    { size: 11 }), k.pos(k.width() / 2 - 10, y), k.anchor('topright'), k.color(240, 200, 80)]);
        k.add([k.text(action, { size: 11 }), k.pos(k.width() / 2 + 10, y), k.anchor('topleft'),  k.color(200, 200, 200)]);
    });
    ...
    if (challenge) {
        ...
        k.add([k.rect(k.width(), 22), k.pos(0, 42), k.color(230, 160, 40), k.fixed(), k.z(10)]);
        k.add([
            k.text(msg, { size: 14, width: k.width() - 40, align: 'center' }),
            k.pos(k.width() / 2, 53),
            k.anchor('center'),
            k.color(20, 20, 20),
            k.z(11),
    ...
    // Blinking prompt
    const prompt = k.add([
        k.text('PRESS SPACE TO START    —    TAB FOR SCOREBOARD', { size: 14 }),
        k.pos(k.width() / 2, k.height() - 12),
        k.anchor('center'),
        k.color(255, 255, 255),
    ]);
    prompt.onUpdate(() => {
...
function gameScene(opts = {}) {
    ...
    const spawnX = opts.from === 'house' ? 930 : 60;
    player.facing = 1;
    ...
    // Sun
    const sunEnt = k.add([k.sprite('sun'), k.pos(360, 30), k.fixed(), k.scale(2), k.z(-90)]);
    ...
    // Parallax clouds that scroll slowly with camera
    /** @type {any[]} */
    const cloudEntities = [];
    for (let i = 0; i < 8; i++) {
        const cloud = k.add([
            k.sprite('cloud'),
            k.pos(200 + i * 360, 20 + (i % 3) * 22),
            k.scale(1 + (i % 2) * 0.5),
            k.z(-70),
            { parallax: 0.3 },
        ]);
    ...
    k.add([
        k.rect(WORLD.width + 400, 80),
        k.pos(-200, WORLD.groundY),
        k.color(110, 74, 40),
        k.area(),
    ...
    // Decorative grass/road tiles on top
    for (let x = -200; x < WORLD.width + 200; x += 16) {
        const inRoad = (x > 60 && x < 260) || (x > 1700 && x < 2000);
        k.add([
    ...
    // Trees + bushes
    for (const [x, y] of [[40, 190], [1250, 190], [2250, 190]]) {
        k.add([k.sprite('tree'), k.pos(x, y), k.anchor('bot'), k.z(-10)]);
    }
    for (const x of [180, 340, 720, 1420, 1620, 2000, 2180]) {
        k.add([k.sprite('bush'), k.pos(x, WORLD.groundY + 2), k.anchor('bot'), k.z(-5)]);
    }

    // Broken pickup truck (spawn area context)
    k.add([k.sprite('truck'), k.pos(10, WORLD.groundY + 2), k.anchor('bot'), k.z(-6)]);
    ...
    const DOOR_W = 10;
    const DOOR_H = 18;
    const DOOR_CENTER_X = DOOR_X + DOOR_W / 2;  // 900
    ...
    const doorHint = k.add([
        k.text('Z/X BREAK', { size: 10 }),
        k.pos(DOOR_CENTER_X, DOOR_Y - 10),
        k.anchor('center'),
        k.color(255, 240, 80),
        k.opacity(0),
        k.z(50),
    ...
    doorHint.onUpdate(() => {
        const dx = Math.abs(DOOR_CENTER_X - (p.pos.x + 8));
        if (dx >= 40) { doorHint.opacity = 0; return; }
    ...
    const { p, spawnFire, spawnEnemy, spawnPickup, spawnPoof, consumePickup, nearestInteractable } =
        buildPlayingContext({ spawnX, minX: 0, maxX: WORLD.width - 16,
            spawnFireAt: (x) => spawnFire(x, WORLD.groundY) });

    // Camera follow — outside scene scrolls horizontally with the player.
    p.onUpdate(() => {
    ...
    spawnPickup('beer',         220, WORLD.groundY - 6, 'beer-road');
    spawnPickup('shotgun',      380, WORLD.groundY - 6, 'shotgun-380');
    spawnPickup('ammo',         480, WORLD.groundY - 5, 'ammo-480');
    spawnPickup('bat',          620, WORLD.groundY - 4, 'bat-620');
    spawnPickup('taser',        820, WORLD.groundY - 6, 'taser-820');
    spawnPickup('ammo',        1120, WORLD.groundY - 5, 'ammo-1120');
    spawnPickup('grenade',     1250, WORLD.groundY - 6, 'grenade-1250');
    spawnPickup('cap',         1550, WORLD.groundY - 3, 'cap-1550');
    spawnPickup('flamethrower',1800, WORLD.groundY - 6, 'flamethrower-1800');
    spawnPickup('ammo',        2000, WORLD.groundY - 5, 'ammo-2000');

    // ----- Enemies (peaceful by default; IDs persist kills across scenes) ---
    spawnEnemy('dog',     420, 'wander',   'dog-420');
    spawnEnemy('child',   700, 'wander',   'child-700');
    spawnEnemy('child',  1280, 'wander',   'child-1280');
    spawnEnemy('mother', 1380, 'peaceful', 'mother-1380');
    spawnEnemy('dog',    1500, 'wander',   'dog-1500');
    spawnEnemy('scout',  1700, 'peaceful', 'scout-1700');
    spawnEnemy('cop',    1860, 'peaceful', 'cop-1860');
    spawnEnemy('mallCop',2050, 'peaceful', 'mallCop-2050');
    spawnEnemy('choir',  2560, 'peaceful', 'choir-2560');
    spawnEnemy('choir',  2580, 'peaceful', 'choir-2580');
    spawnEnemy('choir',  2600, 'peaceful', 'choir-2600');
    spawnEnemy('boss',   2680, 'peaceful', 'boss-2680');
    ...
    k.onKeyPress(['down', 's'], () => {
        const item = nearestInteractable(22, 36);
...
function houseScene() {
    ...
    // Back wall tiles (16×16, repeated across the playable width).
    for (let x = 0; x < 480; x += 16) {
        for (let y = 60; y < WORLD.groundY; y += 16) {
    ...
    k.add([
        k.rect(k.width() + 40, 80),
        k.pos(-20, WORLD.groundY),
        k.color(56, 34, 22),
        k.opacity(0),
        k.area(),
        k.body({ isStatic: true }),
    ...
    k.add([k.sprite('cabinet'), k.pos(420, WORLD.groundY + 2), k.anchor('bot'), k.z(-10)]);
    k.add([k.sprite('table'),   k.pos(240, WORLD.groundY + 2), k.anchor('bot'), k.z(-10)]);

    // Left-side exit door (painted) + invisible trigger in front of it.
    k.add([k.sprite('exitDoor'), k.pos(40, WORLD.groundY + 2), k.anchor('bot'), k.z(-4)]);
    k.add([
        k.rect(30, 44),
        k.pos(20, WORLD.groundY - 44),
        k.opacity(0),
        k.area(),
        'exit',
    ]);
    ...
    const exitHint = k.add([
        k.text('← EXIT', { size: 10 }),
        k.pos(40, WORLD.groundY - 54),
        k.anchor('center'),
        k.color(255, 240, 80),
        k.z(50),
    ]);
    exitHint.onUpdate(() => {
        exitHint.pos.y = WORLD.groundY - 54 + Math.sin(k.time() * 5) * 1.5;
    });
    ...
    const { p, spawnEnemy, spawnPickup, consumePickup, nearestInteractable } =
        buildPlayingContext({ spawnX: 80, minX: 20, maxX: 460 });
    ...
    spawnEnemy('mother',  300, 'hostile', 'mother-house');
    spawnPickup('syringe', 440, WORLD.groundY - 4, 'syringe-house');
...
function debugScene() {
    ...
    k.add([k.sprite('sun'), k.pos(360, 30), k.fixed(), k.scale(2), k.z(-90)]);
    ...
    for (let x = -200; x < k.width() + 200; x += 16) {
        k.add([k.sprite('ground'), k.pos(x, WORLD.groundY), k.z(-19)]);
    }

    k.add([
        k.text('DEBUG', { size: 11 }),
        k.pos(k.width() / 2, 48),
        k.anchor('top'),
        k.fixed(),
        k.color(240, 200, 80),
    ...
    // which is ~400px from the player at screen-left, outside the default 300.
    ...
    function scheduleSpawn() {
        ...
        k.wait(delay, () => {
            if (k.get('enemy').length < MAX_CONCURRENT) {
                ...
                spawnEnemy(kind, k.width() + 20, 'hostile');
            }
            scheduleSpawn();
        });
    }
...
async function endingScene({ stats, player, runTimeMs }) {
    ...
    if (peaceful) {
        ...
        // Little choir line of figures
        for (let i = 0; i < 5; i++) {
            k.add([
                k.sprite('choirIdle'),
                k.pos(k.width() / 2 - 80 + i * 34, 200),
                k.anchor('bot'),
                k.scale(1.2),
            ]);
        }
